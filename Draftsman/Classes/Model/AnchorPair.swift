...
struct AnchorPair<Anchor: LayoutAxisAnchor> {
    ...
    func build(with context: DraftContext, for relation: NSLayoutConstraint.Relation) -> NSLayoutConstraint {
    ...
    func build(with context: DraftContext, for relation: NSLayoutConstraint.Relation, offset: CGFloat) -> NSLayoutConstraint {
        let first = firstAnchor.anchor(from: context)
        let second = secondAnchor.anchor(from: context)
...
struct DimensionAnchorPair {
    ...
    func build(with context: DraftContext, for relation: NSLayoutConstraint.Relation, constant: CGFloat) -> NSLayoutConstraint {
        let first = firstAnchor.anchor(from: context)
        let second = secondAnchor.anchor(from: context)
        return first.constraint(relation, to: second, constant: constant)
    }
    
    func build(with context: DraftContext, for relation: NSLayoutConstraint.Relation, multiplier: CGFloat) -> NSLayoutConstraint {
        let first = firstAnchor.anchor(from: context)
        let second = secondAnchor.anchor(from: context)
        return first.constraint(relation, to: second, multiplier: multiplier)
    }
    
    func build(with context: DraftContext, for relation: NSLayoutConstraint.Relation, multiplier: CGFloat, constant: CGFloat) -> NSLayoutConstraint {
        let first = firstAnchor.anchor(from: context)
        let second = secondAnchor.anchor(from: context)
